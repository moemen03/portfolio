...
const DemoComputer = (props) => {
  ...
  const videoAspect = txt.image?.videoWidth && txt.image?.videoHeight
    ? txt.image.videoWidth / txt.image.videoHeight
    : 16 / 9;
          ...
          <shaderMaterial
            ...
            fragmentShader={`
              ...
              void main() {
                ...
                vec4 barColor = vec4(0.004, 0.004, 0.008, 1.0);

                if (videoAspect < screenAspect) {
                  float contentWidth = videoAspect / screenAspect;
                  float leftEdge = (1.0 - contentWidth) * 0.5;

                  if (screenUv.x < leftEdge || screenUv.x > 1.0 - leftEdge) {
                    gl_FragColor = barColor;
                    return;
                  }

                  fittedUv.x = (screenUv.x - leftEdge) / contentWidth;
                } else if (videoAspect > screenAspect) {
                  float contentHeight = screenAspect / videoAspect;
                  float bottomEdge = (1.0 - contentHeight) * 0.5;

                  if (screenUv.y < bottomEdge || screenUv.y > 1.0 - bottomEdge) {
                    gl_FragColor = barColor;
                    return;
                  }

                  fittedUv.y = (screenUv.y - bottomEdge) / contentHeight;
                }

                gl_FragColor = texture2D(videoMap, fittedUv);
              }
            `}
          />
        </mesh>
